ASSETSFOLFER=assets/timeline

for mediaFile in `ls $ASSETSFOLFER | grep .mp4`; do 
    # cortar a extensão e a resolução
    FILENAME=$(echo $mediaFile | sed -n 's/.mp4//p' | sed -n 's/-1920x1080//p')
    INPUT=$ASSETSFOLFER/$mediaFile
    FOLFER_TARGET=$ASSETSFOLFER/$FILENAME
    mkdir -p $FOLFER_TARGET

    #cirar aquivo de resolução
    OUTPUT=$ASSETSFOLFER/$FILENAME/$FILENAME
